...
backgroundCorrection_FileName = 'Out_1.0Muons_1.0Electrons_100A.root'
backgroundCorrection_File = TFile.Open(backgroundCorrection_FileName)
backgroundCorrection_Int  = backgroundCorrection_File.Get("backgroundCorrection_Int100A")
backgroundCorrection_Inc  = backgroundCorrection_File.Get("backgroundCorrection_Inc100A")
XS_MC = backgroundCorrection_File.Get("MC_PionOnlyRecoXS100A")
XS_MC.SetLineColor(kRed)
XS_MC.SetFillColor(0)

efficiencyCorrection_FileName = 'EfficiencyCorrectionPions100A.root'
efficiencyCorrection_File = TFile.Open(efficiencyCorrection_FileName)
efficiencyCorrection_Int  = efficiencyCorrection_File.Get("multEffCorr_Int_100A")
efficiencyCorrection_Inc  = efficiencyCorrection_File.Get("multEffCorr_Inc_100A")
trueXS  = efficiencyCorrection_File.Get("XSTrue_100A")


pionData_FileName = '/Volumes/Seagate/Elena/TPC/Data100A.root'
...
rawXS.SetLineColor(kBlue)
bkgSubXS.SetLineColor(kBlue-2)
effCorrXS.SetLineColor(kBlue-4)
rawXS.SetMarkerColor(kBlue)
bkgSubXS.SetMarkerColor(kBlue-2)
effCorrXS.SetMarkerColor(kBlue-4)
outFile = TFile("DataCorrected100A.root","recreate")
outFile.cd()
rawXS.Write("pionXS_Raw100A"       ,TObject.kWriteDelete)
bkgSubXS.Write("pionXS_DataBkgSub100A"       ,TObject.kWriteDelete)
effCorrXS.Write("pionXS_DataBkgSubEffCorr100A",TObject.kWriteDelete)
