...
true_FileName = "/Volumes/Seagate/Elena/TPC/TrueXS.root"
...
true_File   = TFile.Open(true_FileName)
...
trueXS      = true_File.Get("TrueXS/hCrossSection")
...
# MC Pi Mu E Cross Section
XSMCReco = totHisto_Int.Clone("XSMCRecoBkg")
XSMCReco.Divide(totHisto_Inc)
XSMCReco.Scale(101.10968)
XSMCReco.Draw("histo")
XSMCReco.SetFillColor(0)
XSMCReco.SetTitle("MC Cross Section; KE [MeV]; Cross Section [barns]")

# MC Pi Cross Section
XSMCRecoPion = pionMC_Int.Clone("pionMCXSMC")
XSMCRecoPion.Divide(pionMC_Inc)
XSMCRecoPion.Scale(101.10968)
XSMCRecoPion.SetLineColor(kRed)
XSMCRecoPion.Draw("samehisto")
XSMCRecoPion.SetFillColor(0)

# Data Pi Cross Section
...
trueXS.SetLineWidth(2)
trueXS.SetLineColor(kBlack)
trueXS.Draw("histosame")

legend1 = TLegend(.54,.52,.84,.70);
legend1.AddEntry(XSMCRecoPion,"MC Pion Only");
legend1.AddEntry(XSMCReco,"MC Pion Muon Electron");
legend1.AddEntry(trueXS,"TruePions");
legend1.AddEntry(XSDataRecoPion,"Data Pion Muon Electron");
legend1.Draw("same")
